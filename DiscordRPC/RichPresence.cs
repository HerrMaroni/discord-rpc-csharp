...
public class BaseRichPresence
{
    ...
    protected internal string InternalDetails;

    /// <summary>Internal inner state string</summary>
    protected internal string InternalState;
    ...
    public string State
    {
        ...
        set
        {
            if (!ValidateString(value, out InternalState, 128, Encoding.UTF8))
    ...
    public string Details
    {
        ...
        set
        {
            if (!ValidateString(value, out InternalDetails, 128, Encoding.UTF8))
    ...
    ///     ids, lobby ids, etc. Make this object null if you do not wish too / unable to implement the Join / Request feature.
    ///     <para>
    ...
    ///     be used as a form of notification, but was replaced with the join feature. It may potentially be useful in the
    ...
    /// <summary>
    ///     Attempts to call <see cref="StringTools.GetNullOrString(string)" /> on the string and return the result, if it's
...
#pragma warning disable CS0618 // Type or member is obsolete
        ...
        if (Party != null)
        {
            if (other.Party == null ||
                ...
                other.Party.CurrentSize != Party.CurrentSize ||
...
public class Secrets
{
    ...
    /// </summary>
    [Obsolete(
        "This feature has been deprecated my Mason in issue #152 on the official library. Was originally used as a Notify Me feature, it has been replaced with Join / Spectate.")]
    ...
    ///     a fancy matchmaker or player id, lobby id, etc.
    ///     <para>
    ///         It is recommended to encrypt this information, so it's hard for people to replicate it.
    ...
    ///     for a fancy matchmaker or player id, lobby id, etc.
    ///     <para>
    ///         It is recommended to encrypt this information, so it's hard for people to replicate it.
    ///         Do <b>NOT</b> just use the IP address in this. That is a bad practice and can leave your players vulnerable!
    ///     </para>
...
public class Assets
{
    ...
    public string LargeImageKey
    {
        ...
        set
        {
            ...
            // Get if this is an external link
...
public class Party
{
    ...
    /// <summary>
    ///     The maximum size of the party / lobby / group. This is required to be larger than <see cref="CurrentSize" />. If it
    ///     is smaller than the current party size, it will automatically be set too <see cref="CurrentSize" /> when the
    ///     presence is sent.
    /// </summary>
    [JsonIgnore]
    ...
    private int[] Size
    {
        get
        {
            ...
            var size = Math.Max(1, CurrentSize);
    ...
    public string Url
    {
        ...
        set
        {
            ...
            if (!Uri.TryCreate(_url, UriKind.Absolute,
                    out _)) // || !(uriResult.Scheme == Uri.UriSchemeHttp || uriResult.Scheme == Uri.UriSchemeHttps))
...
public sealed class RichPresence : BaseRichPresence
{
    ...
    ///     properties if they are in an invalid state.
    /// </summary>
    /// <returns></returns>
    public RichPresence Clone()
    {
        ...
        {
            State = InternalState != null ? InternalState.Clone() as string : null,
            Details = InternalDetails != null ? InternalDetails.Clone() as string : null,
    ...
    {
        InternalState = presence.State;
        InternalDetails = presence.Details;
